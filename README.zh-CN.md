...
把每个 YouTube 视频变成一份可以深入学习的资料。YouTube Note 把字幕、双语翻译、AI 视频简介、章节、重点引言、内容讲解和时间戳笔记整合进同一个 Chrome 侧边栏，界面全中文，让你随时随地把视频内容和语言都学透，同时不丢失观看位置。

- 把零碎字幕整理成清晰、可搜索的学习资料，支持「原文 / 中文 / 双语」三种视图随时切换。
- 字幕、概览、笔记三个标签页都有「原文 / 中文 / 双语」切换按钮，顶部还会同时显示中英双语视频标题。
- 用 AI 自动生成视频简介（内容概览）、章节（带时间戳的中文标题和简介）和重点引言，快速抓住视频脉络。
- 选中字幕中的任何内容即可一键解释，把复杂概念讲明白。
- 点击字幕、概览或笔记中的时间戳，立即跳转到视频对应位置。
- 一键保存带时间戳的精炼笔记，AI 自动润色并附上中文翻译，方便之后复习。
- 全部界面、按钮和状态提示均为简体中文，加载、错误和提示信息一目了然。
- 使用你自己的 API Key，数据保存在本地 Chrome 中，没有分析统计、没有行为追踪。

YouTube Note 是一个自带密钥（BYOD）的开源项目，从 GitHub 本地安装。它没有上架 Chrome 网上应用商店，不包含 API 额度，也不运行开发者运营的服务器。

## 让你的编程 Agent 帮你安装

你不需要看懂代码，也不需要会使用命令行。把下面这段话发送给你的编程 Agent：

> 请把这个项目下载或克隆到我选择的长期保留文件夹，告诉我准确的完整路径，并让 Chrome“加载已解压的扩展程序”使用同一个文件夹。如果我在第一次安装时需要位置建议，可以推荐 macOS 或 Linux 上的 `~/Documents/youtube-note`，或 Windows 上的 `%USERPROFILE%\Documents\youtube-note`，但不要假设我一定使用这些路径。请用简单易懂的语言一步一步指导我完成安装和配置。https://github.com/lilidemgtv-bit/youtube-note

你的 Agent 应该帮你：

1. 先询问你想把项目长期保存在哪里，再下载或克隆到那里，并告诉你准确的完整路径。如果你需要建议，可以推荐 macOS 或 Linux 上的 `~/Documents/youtube-note`，或 Windows 上的 `%USERPROFILE%\Documents\youtube-note`。
...
不要把 API Key 发送到 AI 对话、源代码、截图或公开消息中。请你自己在 YouTube Note 的设置页面直接填写。编程 Agent 可以告诉你填写位置，但不需要看到 Key。

## 手动安装

如果你想自己操作：

1. 打开 [github.com/lilidemgtv-bit/youtube-note](https://github.com/lilidemgtv-bit/youtube-note)。
2. 点击 **Code**，再选择 **Download ZIP**。
3. 选择一个长期保留的文件夹，并把项目解压到这里。可选建议是 macOS 或 Linux 上的 `~/Documents/youtube-note`，或 Windows 上的 `%USERPROFILE%\Documents\youtube-note`。你也可以使用其他文件夹。
4. 在 Chrome 地址栏打开 `chrome://extensions`。
5. 打开右上角的“开发者模式”。
6. 点击“加载已解压的扩展程序”。
7. 选择你刚才确定的那个准确项目文件夹，其中必须包含 `manifest.json`。
8. 如果需要，可以在 Chrome 扩展菜单中固定 YouTube Note。

这是一个本地加载的扩展，不会自动更新。下载新版或让 Agent 修改代码后，请在 `chrome://extensions` 中找到 YouTube Note 并点击“重新加载”，然后刷新已经打开的 YouTube 页面。如果移动或删除源代码文件夹，Chrome 中加载的扩展会失效，需要从新的位置重新加载。

## 设置 API Key

YouTube Note 需要你在自己的服务账号中准备两个 Key：
...
5. 复制 Key，并粘贴到 YouTube Note 设置中的 **Supadata API key**。
...
3. 点击 **Create new API key**，填写容易识别的名称，例如 `YouTube Note`，然后创建 Key。
4. 立即复制 Key。完整 Key 可能只会显示一次。
5. 把 Key 粘贴到 YouTube Note 设置中的 **DeepSeek API key**。
6. 如果 DeepSeek 提示余额不足，请在 DeepSeek 开放平台账号中充值后再试。

当前账号和接口说明请查看 [DeepSeek 官方 API 文档](https://api-docs.deepseek.com/)。

在侧边栏中打开 **Settings**。你也可以在 `chrome://extensions` 的 YouTube Note 卡片中打开扩展选项。Key 只能粘贴到这些设置输入框中。不要把 Key 发送到 AI 对话、项目文件、截图或公开消息中。
...
YouTube Note 会让所有 DeepSeek 请求使用非思考模式，以获得更快、更稳定的交互。设置中的接口地址和模型固定，只需要填写 DeepSeek API Key。如果想使用其他服务或模型，请在设置中复制安全的自定义 prompt，让编程 Agent 修改你自己的本地副本。不要把任何 API Key 放进 prompt 或对话。
...
2. 点击 YouTube Note 扩展图标，打开侧边栏，顶部会显示中英双语视频标题。
3. **字幕**：阅读带时间戳的字幕，随时切换「原文 / 中文 / 双语」。
4. **概览**：查看 AI 自动生成的视频简介、带时间戳的中文章节和重点引言，同样支持「原文 / 中文 / 双语」切换。
5. **笔记**：查看带时间戳的精炼笔记（自动附中文翻译），支持「原文 / 中文 / 双语」切换。
6. 选中字幕中的文本，即可获得 AI 讲解。
7. 从播放器或重点引言中保存笔记，之后可以在「笔记」中查看。

## 当前支持范围

- Chrome 116 或更高版本。
- 标准的 `youtube.com/watch` 视频页面。
- Supadata 能够返回的原生字幕。YouTube Note 会优先请求英文字幕，也可能显示其他可用的原生语言。
- 字幕、概览、笔记三个标签页都支持「原文 / 简体中文 / 双语」视图，顶部显示中英双语视频标题。
- AI 视频简介、章节、引言、选中文本讲解、翻译和自动润色笔记。
...
YouTube Note 强制使用 Supadata 的 `mode=native`，不会在没有原生字幕时请求 AI 生成转录，也不会在本地转录音频。
...
- AI 生成字幕每分钟消耗 **2 credits**。YouTube Note 不会使用这条路径，因为它强制使用 `mode=native`。
- 如果没有可用原生字幕并返回 HTTP `206`，仍会消耗 **1 credit**。

按照当前只获取原生字幕的方式，如果每次请求都成功，免费版每月大约可以查询 100 个视频。重试和没有字幕的查询也会消耗额度，所以实际成功数量可能更少。

DeepSeek 的额度与 Supadata 分开计算。DeepSeek 可能有自己的免费额度、限速或费用。YouTube Note 不收款，也不转售 API 服务。建议为两个账号设置消费上限并定期查看用量。下方估算说明了当前 DeepSeek 翻译成本。
...
一个实测的 20 分钟英文演讲包含 **2,935 个英文口语词**和 15,433 个字幕字符。按 YouTube Note 当前的分组方式，它会变成 128 个语义分段，以每次 3 段的方式发出 43 次请求。算上重复 prompt 和 JSON 后，渲染后的输入约为 108,528 个英文字符，按官方每个英文字符 0.3 token 的经验值，即**约 32,600 个输入 token**。按每个中文字符 0.6 token 的经验值，再加上 JSON 和 ID 开销，中文 JSON 输出估计为 3,500 到 4,500 token。
...
YouTube Note 使用原生 HTML、CSS 和 JavaScript，没有构建步骤，很适合用编程 Agent 做个人项目。你可以尝试：
...
如果想使用其他 AI 服务或模型，请先在编程 Agent 中打开 Chrome 通过“加载已解压的扩展程序”使用的那个准确的 YouTube Note 项目文件夹。然后打开 YouTube Note 设置并点击 **Copy customization prompt**。发送前替换 `[PROVIDER]` 和 `[MODEL]`，但不要加入任何 API Key。Agent 完成本地代码修改后，请你自己在它指出的设置位置填写 Key。

## 隐私和数据流向

YouTube Note 会直接从扩展向服务商发送请求：
...
YouTube Note 没有账号系统、广告、分析统计或行为追踪。Supadata 和 DeepSeek 仍会按照各自的条款和隐私政策处理数据。详情请查看 [PRIVACY.md](PRIVACY.md)。

## 常见问题

### YouTube 视频页面没有显示 Digest 按钮

- 在 `chrome://extensions` 中找到 YouTube Note，点击“重新加载”，然后刷新 YouTube 页面。
...
- 在 `chrome://extensions` 中确认 YouTube Note 已启用，并点击“重新加载”。
