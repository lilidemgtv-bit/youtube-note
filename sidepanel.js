...
let currentNotesMode = "zh";
let currentNotesList = [];
let currentNotesFilter = null;
...
function setupEventListeners() {
  ...
  // Overview / Notes language mode buttons
...
function renderAnalysisResults(analysis) {
  // Bilingual video title in the header
  const titleZhEl = document.getElementById("videoTitleZh");
  if (titleZhEl) {
    titleZhEl.textContent = analysis?.titleZh || "";
  }

  // Video summary
  const summaryEl = document.getElementById("videoSummary");
  if (summaryEl) {
    const summary = pickModeText(analysis?.summary, analysis?.summaryZh, currentOverviewMode);
    summaryEl.textContent = summary || "暂无简介";
  }
  // Chapters
  ...
  (analysis.chapters || []).forEach((chapter) => {
    ...
    li.innerHTML = `
      ...
      <div class="chapter-content">
        <span class="chapter-title">${escapeHtml(pickModeText(chapter.title, chapter.titleZh, currentOverviewMode))}</span>
        <span class="chapter-summary">${escapeHtml(pickModeText(chapter.summary, chapter.summaryZh, currentOverviewMode))}</span>
      </div>
    `;
  ...
  sortedQuotes.forEach((quote) => {
    ...
    div.innerHTML = `
      <div class="quote-text">${escapeHtml(pickModeText(quote.quote, quote.quoteZh, currentOverviewMode))}</div>
...
async function triggerAnalysis() {
  ...
  const quotesList = document.getElementById("quotesList");
  const summaryEl = document.getElementById("videoSummary");
  if (summaryEl) summaryEl.textContent = "正在生成视频简介...";
...
/**
 * Renders saved notes into the Notes tab.
 */
function renderNotes(notes, filteredVideoId) {
  ...
  currentNotesFilter = filteredVideoId;

  list.innerHTML = "";
  if (!notes || notes.length === 0) {
    if (intro) {
      intro.style.display = "block";
      intro.textContent = filteredVideoId
        ? "这个视频还没有笔记。播放时暂停视频并点击 📝 笔记 按钮，或按 “n” 键保存。"
        : "还没有保存任何笔记。播放时暂停视频并点击 📝 笔记 按钮，或按 “n” 键保存。";
    }
    return;
  }
  ...
  notes.forEach((note) => {
    list.appendChild(
      createNoteElement(note, filteredVideoId, currentNotesMode, () => {
...
/**
 * Builds a note item element with all its actions wired up.
 */
function createNoteElement(note, filteredVideoId, mode, onDelete) {
  ...
  noteEl.innerHTML = `
      ...
      <div class="note-text">"${escapeHtml(pickModeText(note.text, note.textZh, mode))}"</div>
  ...
  // 复制文本 button — copies just the note's text
  noteEl
  ...
  // 复制时间戳 button — copies the timestamped YouTube link
  noteEl
...
function pickModeText(original, translated, mode) {
  if (mode === "original") return original || translated || "";
  if (mode === "bilingual") {
...
function handleNotesModeChange(mode) {
  ...
  renderNotes(currentNotesList, currentNotesFilter);
}
