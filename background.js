...
function validateAndFixTimestamps(analysis, maxSeconds) {
  ...
  return {
    title: safeString(analysis?.title, 500),
    titleZh: safeString(analysis?.titleZh, 500),
    summary: safeString(analysis?.summary, 2000),
    summaryZh: safeString(analysis?.summaryZh, 2000),
    chapters,
    keyQuotes,
    keyMoments,
  };
...
async function handleSaveNote(
  ...
  try {
    ...
    const cleaned = await cleanupNoteText(
      matchedLine.text,
      beforeLine,
      afterLine,
      contextLines.join(" "),
    ...
    const note = {
      ...
      text: cleaned.quote,
      textZh: (cleaned && cleaned.quoteZh) || "",
...
async function cleanupNoteText(
  ...
  if (!settings.aiApiKey) {
    return {
      quote: [beforeText, targetText, afterText].filter(Boolean).join(" "),
      quoteZh: "",
    };
  }
  ...
  try {
    ...
    try {
      ...
      if (typeof parsed.quote === "string" && parsed.quote.trim()) {
        return {
          quote: parsed.quote.trim().slice(0, 3000),
          quoteZh:
            typeof parsed.quoteZh === "string" && parsed.quoteZh.trim()
              ? parsed.quoteZh.trim().slice(0, 3000)
              : "",
        };
      }
    } catch (parseError) {
